...
function resolveCharFile(): string {
  const candidates = [
    path.resolve(
      __dirname,
      '..',
...
interface ActorState {
  ...
  stabilized: boolean;
  deathFails: number;
  // protectCount: number of incoming attacks that will be at disadvantage
  protectCount: number;
  // mpShieldCount: number of incoming attacks that will have damage halved due to recent MP-use
  mpShieldCount: number;
}
...
function readCharacters(): RawChar[] {
  ...
  if (fs.existsSync(CACHE_FILE)) {
    try {
      ...
      console.warn(
        'Failed to read cache, will load TS source:',
        (e as Error).message,
      );
    }
  }
  ...
  if (idx === -1) throw new Error('characterList export not found in source');
  ...
  try {
    ...
    throw new Error(
      'Failed to load characterList from TS source: ' + (e as Error).message,
    );
  }
  try {
    fs.unlinkSync(tmpFile);
  ...
  if (!Array.isArray(list)) throw new Error('characterList was not an array');
  const chars = list
    .map((c) => {
      ...
      return {
        ...
        stats: c.stats || {},
        ability: (c as any).ability,
        weakness: (c as any).weakness,
      };
    })
...
function deriveStats(stats: Record<string, number>, opts: { mpStat?: string }) {
  ...
  const STR = ((stats as any)['strength'] ?? 0) - 3;
  const DEX = ((stats as any)['dexterity'] ?? 0) - 3;
  const CON = ((stats as any)['constitution'] ?? 0) - 3;
  const WIS = ((stats as any)['wisdom'] ?? 0) - 3;
  const INT = ((stats as any)['intelligence'] ?? 0) - 3;
  const CHR = ((stats as any)['charisma'] ?? 0) - 3;
...
function runPairwise(
  chars: any[],
  options: any,
): { results: Record<string, any>; winners: Record<string, number>[] } {
    ...
    for (let j = i + 1; j < chars.length; j++) {
      ...
      for (let t = 0; t < options.trials; t++) {
        ...
        if (res.winner === a.name) {
          results[key].aWins++;
          winners.push(a.raw || a.derived?.raw || a.derived?.rawStats || a);
        } else if (res.winner === b.name) {
          results[key].bWins++;
          winners.push(b.raw || b.derived?.raw || b.derived?.rawStats || b);
...
function simulateEngine(aBase: CharEntry, bBase: CharEntry, opts: SimOptions) {
  const log: string[] = [];
  const a: ActorState = {
  ...
  log.push(
    `=== START: ${a.name} vs ${b.name} (HP ${a.hp} / ${b.hp}, MP ${a.mp} / ${b.mp}) ===`,
  );
  let turn = 0;
  while (turn < opts.maxTurns) {
    turn++;
    log.push(`-- Turn ${turn} --`);
    if (!a.down) actionWithLog(a, b, aBase.derived, opts, log);
    ...
    if (!b.down) actionWithLog(b, a, bBase.derived, opts, log);
    if (immediateLossOnZero) {
      if (a.hp <= 0) {
  ...
  function actionWithLog(
    actor: ActorState,
    target: ActorState,
    actorDerived: DerivedStats,
    options: SimOptions,
    out: string[],
  ) {
    if (actor.down) return;
    const r = Math.random();
    const mpProb = options.mpUsageProb ?? DEFAULTS.mpUsageProb;

    // Determine the physical attack modifier (best of STR/DEX as exposed by dmgMod)
    const physMod = Math.max(
      actorDerived.dmgMod || 0,
      actorDerived.dexMod || 0,
    );
    ...
    let chosen: 'mp' | 'defend' | 'evade' | 'attack';
    if (preferMP) chosen = 'mp';
    else if (actor.mp > 0 && r < mpProb) chosen = 'mp';
    else if (r >= mpProb && r < mpProb + 0.05) chosen = 'defend';
    else if (r >= mpProb + 0.05 && r < mpProb + 0.1) chosen = 'evade';
    else chosen = 'attack';
    ...
    if (chosen === 'mp') {
      ...
      out.push(
        `${actor.name} spends 1 MP to make an MP-attack (MP left=${actor.mp}); MP-shield granted (1 incoming hit will be halved)`,
      );
      ...
      if (target.evade) {
        ...
        out.push(
          `${actor.name} MP-attacks (evade penalty -> min(${r1},${r2})=${attackRoll})`,
        );
      } else {
      ...
      out.push(
        `  MP-attack total ${attackRoll}+mpMod+1 ${attackMod} = ${attackTotal} vs AC ${target.AC} => ${
          hit ? 'HIT' : 'MISS'
        }`,
      );
      if (hit) {
        ...
        out.push(
          `  -> ${actor.name} deals ${dmg} damage to ${target.name} (HP now ${target.hp})`,
        );
        if (mpShieldUsed) {
          out.push(
            `${target.name}'s MP-shield consumed (remaining ${target.mpShieldCount})`,
          );
        }
        target.defend = false;
        target.evade = false;
    ...
    if (target.evade) {
      ...
      attackRoll = Math.min(r1, r2);
      out.push(
        `${actor.name} attacks (evade penalty -> min(${r1},${r2})=${attackRoll})`,
      );
    } else {
    ...
    const hit = attackTotal > target.AC;
    out.push(
      `  attack total ${attackRoll}+atkMod ${attackMod} = ${attackTotal} vs AC ${target.AC} => ${hit ? 'HIT' : 'MISS'}`,
    );
    if (hit) {
      let dmg = Math.max(1, 1 + actor.dmgMod);
      if (target.defend) dmg = Math.floor(dmg / 2);
      localApplyDamage(target, dmg);
      out.push(
        `  -> ${actor.name} deals ${dmg} damage to ${target.name} (HP now ${target.hp})`,
      );
  ...
  function localApplyDamage(target: ActorState, dmg: number) {
    target.hp -= dmg;
    if (target.hp <= 0) target.down = true;
  }

  function handleDeathSaveLogging(
    character: ActorState,
    options: SimOptions,
    out: string[],
  ) {
    if (!character.down) return;
    const roll = rollD10();
    if (options.immediateLoss === true) {
      out.push(
        `${character.name} is down; immediate loss mode => no death-save`,
      );
    } else {
      ...
      if (roll > 5) {
        ...
        out.push(
          `${character.name} failed death-save (${character.deathFails}/3)`,
        );
      }
    }
  }
...
function summarize(
  results: Record<
    string,
    { turns: number[]; aWins: number; bWins: number; draws: number }
  >,
) {
  const summary: {
    matchup: string;
  ...
  for (const [k, v] of Object.entries(results)) {
    const avgTurns =
      v.turns.reduce((s: number, x: number) => s + x, 0) /
      (v.turns.length || 1);
    summary.push({
      matchup: k,
      aWins: v.aWins,
...
function main() {
  ...
  const chars = charsRaw.map((c) => {
    ...
    const adjusted: Record<string, number> = { ...(c.stats || {}) };
    try {
      if (
        c.ability &&
  ...
  if (logIndex !== -1) {
    ...
    console.log(
      `Logging sample fights for ${subject.name} vs random opponents:`,
    );
    for (let i = 0; i < 3; i++) {
  ...
  console.log(
    `Parsed ${chars.length} characters. Running pairwise ${trials} trials per match (immediateLoss=${immediate})`,
  );
  ...
  const allTurns = Object.values(results).flatMap(
    (v: { turns: number[] }) => v.turns as number[],
  );
  const globalAvgTurns =
    allTurns.reduce((s: number, x: number) => s + x, 0) /
    (allTurns.length || 1);
  ...
  const aggregate: Record<
    string,
    { wins: number; games: number; totalTurns: number; countMatches: number }
  > = {};
  ...
  for (const [k, v] of Object.entries(results)) {
    ...
    (aggregate[a].totalTurns as number) = (aggregate[a].totalTurns || 0) + avg;
    (aggregate[a].countMatches as number) =
      (aggregate[a].countMatches || 0) + 1;
    (aggregate[b].totalTurns as number) = (aggregate[b].totalTurns || 0) + avg;
    (aggregate[b].countMatches as number) =
  ...
  for (const k of statKeys) statAcc[k] = [];
  const spreadCounts: Record<string, number> = {};
  for (const w of winners) {
    const parts: string[] = [];
    for (const k of statKeys) {
      const v = (w as Record<string, number>)[k] ?? 0;
      statAcc[k].push(v);
      parts.push(String(v));
    }
  ...
  const statSummary: Record<string, { mean: number; median: number }> = {};
  for (const k of statKeys) {
    const arr = statAcc[k].slice().sort((a, b) => a - b);
    const mean = arr.reduce((s, x) => s + x, 0) / (arr.length || 1);
    const mid = Math.floor(arr.length / 2);
    const median =
      arr.length % 2 === 1 ? arr[mid] : (arr[mid - 1] + (arr[mid] || 0)) / 2;
  ...
  const rosterCounts: Record<string, Record<number, number>> = {};
  for (const k of statKeys) rosterCounts[k] = {};
  for (const c of chars) {
    for (const k of statKeys) {
      const v = (c.raw as Record<string, number>)[k] ?? 0;
      rosterCounts[k][v] = (rosterCounts[k][v] || 0) + 1;
    }
  }
  const winnerCounts: Record<string, Record<number, number>> = {};
  for (const k of statKeys) winnerCounts[k] = {};
  for (const w of winners) {
    for (const k of statKeys) {
      const v = (w as Record<string, number>)[k] ?? 0;
  ...
  console.log(
    'Columns: stat | offset | value | winner% (cumulative) | roster% (cumulative) | diff%pts | mult_vs_baseline',
  );
  for (const k of statKeys) {
    const lines: {
      stat: string;
